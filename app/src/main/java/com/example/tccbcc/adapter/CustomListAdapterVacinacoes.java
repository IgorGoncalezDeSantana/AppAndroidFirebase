...
public class CustomListAdapterVacinacoes extends com.example.tccbcc.adapter.CustomListAdapter {
    public CustomListAdapterVacinacoes(@NonNull Context context, int resource, @NonNull ArrayList<CustomListAdapterClass> objects) {
    ...
    public View getView(final int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        btnExcluir.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                if (user == null ||
                        user.getPet() == null ||
                        user.getPet().getVacinacoes() == null ||
                        user.getPet().getVacinacoes().size() == 0){
                    return;
                }

                user.getPet().removeVacina(user.getPet().getVacinacoes().get(position));
